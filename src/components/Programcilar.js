...
export default function Programcilar() {
  ...
  const [listBest, setListBest] = useState(enIyilerListesi);
  ...
  const oneCikaninIsmi = () => {
    ...
    return listBest.find((programmer) => {
        return programmer.id === featured
    }).isim;
  };
      ...
      <div className='programmers'>
        {
          ...
          listBest.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setFeatured(dev.id)/* burada dev.id 'yi öne çıkan id'ye atayın */ }}>Kutla</button>
            </div>
          )
        }
      </div>
